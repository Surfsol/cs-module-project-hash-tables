...
function sentence(text){
    //put words into array, separate whitespace
    arr = text.replace(/:|,|'|\.|!|"|\)|\(/g, "")
    arr = arr.split(' ')
    for(let i=0; i<(arr.length - 1); i++){
        let allKeys = Object.keys(arr)
        if (!allKeys.includes(arr[i])){
            cache[arr[i]] = [arr[i+1]]
        } else {
            cache[arr[i]].push(arr[i+1])
        }
    }
    console.log(cache)
}
...
// Your code here
text = `One thing was certain, that the white kitten had had nothing to do with
